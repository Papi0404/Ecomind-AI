...
function getApiKeys(): string[] {
  ...
  if (process.env.GEMINI_API_KEY) {
    // Support comma-separated keys in the main variable
  ...
  // Support separate backup env variables (GEMINI_API_KEY_BACKUP1, GEMINI_API_KEY_BACKUP2, etc.)
  for (let i = 1; i <= 5; i++) {
...
// 1. Call Gemini API for Chat with search engine grounding support and key failover
export async function getAIChatResponse(
  ...
  return executeWithFallback(async (apiKey) => {
    ...
    // First try with Google Search grounding enabled to search various sources
    try {
      ...
      return result.response.text() || 'Maaf, saya tidak dapat memproses jawaban Anda saat ini.';
    } catch (err) {
      console.warn('Gemini search tool activation failed, falling back to standard generation.', err);
      // Fallback: standard run with same key
      const model = genAI.getGenerativeModel({
        model: 'gemini-1.5-flash',
        systemInstruction: SYSTEM_PROMPT,
      });
...
Pastikan hanya mengembalikan JSON yang valid, tanpa penjelasan markdown tambahan di luar JSON.`;
    ...
    try {
      ...
      // Fallback: standard verification with same key
...
export async function summarizeProcedure(text: string): Promise<string> {
  return executeWithFallback(async (apiKey) => {
    const genAI = new GoogleGenerativeAI(apiKey);
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
    const prompt = `Ringkas teks prosedur layanan publik/dokumen resmi berikut menjadi langkah-langkah yang sangat sederhana, mudah dipahami masyarakat awam, dan terstruktur. Gunakan Bahasa Indonesia:\n\n${text}`;
    const result = await model.generateContent(prompt);
    return result.response.text() || 'Gagal meringkas dokumen.';
  }, () => {
